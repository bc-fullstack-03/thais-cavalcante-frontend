...
function UpdatePostModal({ setIsModalOpen, post }: UpdatePostModalProps) {
  const { getFeed } = useContext(FeedContext);
  const [isTextPost, setIsTextPost] = useState<boolean>(!post.image);
  const [isImagePost, setIsImagePost] = useState<boolean>(post.image);
  ...
  function handlePostTitleChange(event: React.ChangeEvent<HTMLInputElement>) {
  ...
  async function handleUpdatePost(event: FormEvent<PostFormElement>) {
    ...
    const updatedPost = await updatePost(post._id, formData, authHeader);
  ...
  return (
    <Dialog.Portal>
      ...
      <Dialog.Content className="fixed bg-black pb-8 px-8 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-96 shadow-lg shadow-black/25">
        ...
        <form onSubmit={handleUpdatePost} className="flex flex-col mt-5">
